...
const Dashboard: React.FunctionComponent = () => {
  ...
  function handleAddToFavorites(character: CharacterInfo) {
    const existingElement = favorited.some(
      char => char.results[0].name === character.results[0].name,
    );
  ...
  return (
    <>
      ...
      >
        <Header>Star Wars Explorer</Header>
        ...
        {characters && (
          <Info>
            {characters.map(character => (
              <>
                <Link
                  key={character.results[0].name}
                  to={`/character/${character.results[0].name}`}
                >
                  <div>
                    <strong className="NormalTitle">
